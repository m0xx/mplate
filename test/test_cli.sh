...
echo "Welcome {{ username }}!" | mplate -f - --context "{\"username\": \"admin\"}" --engine handlebars > test/results/handlebar.txt
diff test/expected/handlebar.txt test/results/handlebar.txt

echo "Welcome <%= user.name %>!" | mplate -f - --context-file test/context.yaml --context-format yaml > test/results/context.txt
